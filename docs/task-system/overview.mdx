...
description: "Understanding (TBD)"
---

All file processing in Chunkr is handled through a task-based system. When you submit a file, a new task is created, and you receive a `task_id`. Use this ID to "poll" (or repeatedly check) the task's status until it's complete.
...
- **Full Task Control**: Manage jobs and customize results with our [CRUD operations](./CRUD.mdx).
...
if task.status == "Succeeded":
    ...
    print(f"Task failed with message: {task.message}")
