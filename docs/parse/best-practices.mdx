...
If you're using Chunkr for data extraction, document analysis, or other non-RAG workflows, you may want to disable chunking entirely. When chunking is disabled, each chunk in the output will contain exactly one segment that represents a full page.
...
<CodeGroup>
  ```python Python
  from chunkr_ai import Chunkr
  from chunkr_ai.types.tasks.parse_create_params import ChunkProcessing
  ...
  # Access individual segments directly
  if task.status == "Succeeded":
      # Each chunk will contain exactly one segment
  ...
  if (completedTask.status === "Succeeded") {
      ...
      for (const chunk of completedTask.output.chunks) {
          const segment = chunk.segments[0];
          console.log(`Segment type: ${segment.segment_type}`);
