...
Each `segment` object contains rich information. At the `chunk` level, corresponding fields are concatenated from all of the segments within that chunk. Here are the most important fields, explained from the perspective of a segment.
...
Beyond the key fields, the Parse output is enriched with additional metadata at the file, page, and segment levels. This includes details like the original file and a PDF version of it, cropped images for segments, and word-level bounding-boxes. 

For a comprehensive breakdown of every field, you can refer to our [API Reference](/api-references). 
