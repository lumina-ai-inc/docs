...
default_config = Configuration(
    segment_processing=SegmentProcessing(
        ...
        Formula=default_table_config,
    )
)
```

```python Pages
# Pages by default are processed using LLM and generate Markdown.

default_page_config = GenerationConfig(
    format=SegmentFormat.MARKDOWN,
    strategy=GenerationStrategy.LLM,
    crop_image=CroppingStrategy.AUTO,
    llm=None,
...
# All other element's content is processed using heuristics.
