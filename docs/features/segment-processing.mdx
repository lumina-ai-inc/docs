...
This is how you can access the `cropped_image` field in the `segment` object:
...
>**Note**: This is the only configuration option that effects the `chunkr` object rather than `segment` object.
...
- Add the LLM content for `Table` to the `embed` field of the chunk, with the LLM content appearing first
