...
*   **`target_length`**: The desired size of each chunk in tokens. Default is `4096`. Set to `0` to disable chunking entirely (useful for extraction workflows).
...
For extraction and non-RAG workflows, you can disable chunking by setting `target_length` to `0`:
...
task = client.tasks.parse.create(
    file="path/to/document.pdf",
    chunk_processing=ChunkProcessing(
        target_length=0  # Disables chunking - one segment per chunk
    )
)
```
